...
class UddoktaPayInternational
{
    ...
    public static function init()
    {
        if (self::$instance == null) {
            self::$instance = new UddoktaPayInternational;
        }
    ...
    {
        $this->total = ($this->due + $this->fee) / $this->convoRate;
    }
    ...
    public function createPayment()
    {
        ...
        try {
            $response = $this->initPayment($fields, 'checkout-v2/global');
...
$UddoktaPay = UddoktaPayInternational::init();
if (!$UddoktaPay->isActive) {
    die("The gateway is unavailable.");
}

$action = $UddoktaPay->request->get('action');
$invid = $UddoktaPay->request->get('id');
...
if ($action === 'verify') {
    $invoiceId = $UddoktaPay->request->get('invoice_id');
