{if $registrarcustombuttonresult=="success"}
    {include file="$template/includes/alert.tpl" type="success" msg="{lang key='moduleactionsuccess'}" textcenter=false}
{elseif $registrarcustombuttonresult}
    {include file="$template/includes/alert.tpl" type="error" msg="{lang key='moduleactionfailed'}" textcenter=false}
{/if}
...
{if $unpaidInvoice}
    <div class="alert alert-{if $unpaidInvoiceOverdue}danger{else}warning{/if}" id="alert{if $unpaidInvoiceOverdue}Overdue{else}Unpaid{/if}Invoice">
        <div class="float-right">
            <a href="viewinvoice.php?id={$unpaidInvoice}" class="btn btn-sm btn-default">
                {lang key='payInvoice'}
            </a>
        </div>
...
<div class="tab-content margin-bottom">
            ...
            <div class="card-body tt-overview">
                {if $alerts}
                    {foreach $alerts as $alert}
                        {include file="$template/includes/alert.tpl" type=$alert.type msg="<strong>{$alert.title}</strong><br>{$alert.description}" textcenter=false}
                    {/foreach}
                {/if}
        ...
        <div class="card tt-custom-card">
            <div class="card-body">
                ...
                {if $changeAutoRenewStatusSuccessful}
                    {include file="$template/includes/alert.tpl" type="success" msg="{lang key='changessavedsuccessfully'}" textcenter=false}
                {/if}

                <p>{lang key='domainrenewexp'}</p>

                <h4 class="h6 mb-4">{lang key='domainautorenewstatus'}: <span class="label label-{if $autorenew}success{else}danger{/if}">{if $autorenew}{lang key='domainsautorenewenabled'}{else}{lang key='domainsautorenewdisabled'}{/if}</span></h4>
    ...
    <div class="tab-pane fade" id="tabNameservers">
        ...
        <div class="card tt-custom-card">
            <div class="card-body">
                ...
                {if $nameservererror}
                    {include file="$template/includes/alert.tpl" type="error" msg=$nameservererror textcenter=false}
                {/if}
    ...
    <div class="tab-pane fade" id="tabAddons">
        <h3 class="card-title mb-1">{lang key='domainaddons'}</h3>
